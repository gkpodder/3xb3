...
station_coordinates = {r['id']: (r['latitude'], r['longitude'])
                       for i, r in stationslist.iterrows()}
...
def make_h(stations, station_coordinates):
    ...
    for source in stations:
        for goal in stations:
            if source != goal:
                precalculated_heuristics[(source, goal)] = heuristic(
                    source, goal, station_coordinates)
...
def experiment_every_pair():
    ...
    result_a_star = timeit.timeit(lambda: timing_exp(
        stations, graph_of_London, h), number=1)
    result_dj = timeit.timeit(lambda: dj_timing(
        graph_of_London, stations), number=1)
    print(result_a_star)
    print(result_dj)
...
def find_lines():
    ...
    for line, setting in connections.groupby('line'):
        lines_dictionary[line] = set(
            setting['station1']).union(set(setting['station2']))
...
def experiment_multiple_transfers():
    ...
    transfer_stations_pairs = [(132, 268), (165, 216), (206, 217), (266, 268), (
        68, 283), (16, 93), (53, 160), (144, 278), (185, 279), (218, 227)]
    ...
    plt.ylabel("Total Time (seconds)")
    plt.title(
        "Performance Comparison of A* and Dijkstra for Stations Requiring Transfers")
    plt.legend()
    plt.show()
...
"""
Potential Problems:
We used time as the weights but some people use the long and lat as the edge weights
2. A* implementation might not be accurate
"""
